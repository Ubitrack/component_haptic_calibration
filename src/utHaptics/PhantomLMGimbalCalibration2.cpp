...
#include "PhantomLMGimbalCalibration2.h"
#include <utHaptics/Function/PhantomFWKOrientationError2.h>
...
Math::Matrix< typename std::iterator_traits< ForwardIterator1 >::value_type::value_type , 3, 3 >
    computePhantomLMGimbalCalibrationImp2(const ForwardIterator1 iJointAnglesBegin, 
										 ...
										 const typename std::iterator_traits< ForwardIterator1 >::value_type::value_type optimizationStepFactor)
{
	...
	Function::PhantomFWKOrientationError2< Type, ForwardIterator1, ForwardIterator2, ForwardIterator3 > func( iJointAnglesBegin, iJointAnglesEnd, iGimbalAnglesBegin, iZRefBegin, l1, l2, angle_correction, calib );
	...
	LOG4CPP_INFO( logger, "PhantomGimbalCalibration Optimization result (residual): " << double(residual)
		<< std::endl << "O4^2 factor: " << parameters(0)  << "O4 factor: " << parameters(2) << " offset: " << parameters(4)
		<< std::endl << "O5^2 factor: " << parameters(1)  << "O5 factor: " << parameters(3) << " offset: " << parameters(5)
		//<< std::endl << "O4 factor: " << parameters(0)  << "O6 factor: " << parameters(2) << " offset: " << parameters(5)
	);	
...
Math::Matrix< float, 3, 3 > computePhantomLMGimbalCalibration2( const std::vector< Math::Vector< float, 3 > > & jointangles,
															  ...
															  const float optimizationStepSize, const float optimizationStepFactor)
{
	...
	return computePhantomLMGimbalCalibrationImp2(jointangles.begin(), jointangles.end(), gimbalangles.begin(), zref.begin(), l1, l2, angle_correction, calib, optimizationStepSize, optimizationStepFactor);
}

Math::Matrix< double, 3, 3 > computePhantomLMGimbalCalibration2( const std::vector< Math::Vector< double, 3 > > & jointangles,
