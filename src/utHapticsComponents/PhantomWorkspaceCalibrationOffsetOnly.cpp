 ...
 *
 */
class PhantomWorkspaceCalibrationOffsetOnly
...
public:
	 ...
	 */
	PhantomWorkspaceCalibrationOffsetOnly( const std::string& sName, boost::shared_ptr< Graph::UTQLSubgraph > config )
	...
	void compute( Measurement::Timestamp ts )
	{
		...
		Math::Matrix< double, 3, 3 > corrFactors = Haptics::computePhantomLMCalibrationOffsetOnly( *m_inAngles.get(), *m_inPositions.get(), m_dJoint1Length, m_dJoint2Length, m_dOriginCalib, m_optimizationStepSize, m_optimizationStepFactor );

		LOG4CPP_DEBUG( logger, "result of computePhantomLMCalibrationOffsetOnly:" <<  corrFactors);
...
UBITRACK_REGISTER_COMPONENT( Dataflow::ComponentFactory* const cf ) {
	cf->registerComponent< PhantomWorkspaceCalibrationOffsetOnly > ( "PhantomWorkspaceCalibrationOffsetOnly" );
}
