...
#include <utHaptics/PhantomLMCalibration2.h>
 ...
 *
 */
class PhantomWorkspaceCalibration2
...
public:
	 ...
	 */
	PhantomWorkspaceCalibration2( const std::string& sName, boost::shared_ptr< Graph::UTQLSubgraph > config )
	...
	void compute( Measurement::Timestamp ts )
	{
		...
		Math::Matrix< double, 3, 3 > corrFactors = Haptics::computePhantomLMCalibration2( *m_inAngles.get(), *m_inPositions.get(), m_dJoint1Length, m_dJoint2Length, m_dOriginCalib, m_optimizationStepSize, m_optimizationStepFactor );
...
UBITRACK_REGISTER_COMPONENT( Dataflow::ComponentFactory* const cf ) {
	cf->registerComponent< PhantomWorkspaceCalibration2 > ( "PhantomWorkspaceCalibration2" );
}
