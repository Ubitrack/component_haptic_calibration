...
#include <utHaptics/PhantomLMGimbalCalibration2.h>
 ...
 *
 */
class PhantomWorkspaceGimbalCalibration2
...
public:
	 ...
	 */
	PhantomWorkspaceGimbalCalibration2( const std::string& sName, boost::shared_ptr< Graph::UTQLSubgraph > config )
	...
	void compute( Measurement::Timestamp ts )
	{
		...
		Math::Matrix< double, 3, 3 > corrFactors = Haptics::computePhantomLMGimbalCalibration2( *m_inJointAngles.get(), *m_inGimbalAngles.get(), *m_inZRef.get(), m_dJoint1Length, m_dJoint2Length, ac, m_dOriginCalib, m_optimizationStepSize, m_optimizationStepFactor );
...
UBITRACK_REGISTER_COMPONENT( Dataflow::ComponentFactory* const cf ) {
	cf->registerComponent< PhantomWorkspaceGimbalCalibration2 > ( "PhantomWorkspaceGimbalCalibration2" );
}
